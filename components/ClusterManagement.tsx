    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">统一管理分布在全国各地的 GPU 算力资源池。</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {MOCK_CLUSTERS.map((cluster) => (
          <div 
            ...
            className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all cursor-pointer relative group"
          >
            <div className="flex items-start justify-between mb-6">
              <div className={`p-3 rounded-2xl ${
                cluster.status === 'Healthy' ? 'bg-green-50 text-green-600' : 
                cluster.status === 'Warning' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'
              }`}>
                <Server size={24} />
              </div>
            ...
            <h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition-colors flex items-center">
              {cluster.name}
              <ChevronRight size={16} className="ml-1 opacity-0 group-hover:opacity-100 transition-all" />
            </h3>
            
            <div className="flex items-center space-x-4 text-xs text-gray-400 mt-2 mb-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between text-xs mb-2">
                  <span className="text-gray-500 font-medium">GPU 资源池占用</span>
                  <span className="font-bold text-blue-600">{Math.round((cluster.usedGpu / cluster.totalGpu) * 100)}%</span>
                </div>
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-700 ${
                      (cluster.usedGpu / cluster.totalGpu) > 0.8 ? 'bg-orange-500' : 'bg-blue-600'
                    }`}
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 text-xs text-gray-400 mt-1">
              <span>{cluster.region}</span>
              <span>•</span>
              <span className="text-green-500 font-bold uppercase">{cluster.status}</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: '平均 GPU 使用率', value: '67.2%', icon: <Zap className="text-orange-500" />, bg: 'bg-orange-50' },
          ...
          { label: '算力健康得分', value: '98', icon: <CheckCircle2 className="text-indigo-500" />, bg: 'bg-indigo-50' },
        ].map((stat, i) => (
              ...
              <tr key={node.id} className="text-sm hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4">
                   <div className="flex items-center space-x-2">
                     <span className="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded text-[10px] font-bold">{node.gpuCount} * A100</span>
                   </div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center space-x-2">
